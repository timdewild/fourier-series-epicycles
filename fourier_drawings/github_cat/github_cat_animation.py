...
coeff_list = np.load(f'fourier_drawings/github_cat/github_cat_coefficients_{order}.npy')
...
canvas.set_axis_properties(aspect = 'equal', xticklabels = [], yticklabels = [])
...
canvas.add_artist(trace, in_legend = True)

canvas.construct_legend()

#---- Animation ----#
animation = Animation(canvas, interval = 10)
animation.render(f'fourier_drawings/github_cat/github_cat_animation_{order}.mp4')